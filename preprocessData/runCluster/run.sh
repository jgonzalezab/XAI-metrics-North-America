...
mkdir -p "${DIR_TO_MOUNT}/preprocessData/cluster-log/"

# SLURM parameters
SLURM_PARAMS="--job-name=${JOB_NAME} --partition=wngpu --gres=gpu:2
              --time=72:00:00 --mem-per-cpu=128000 --output=${DIR_TO_MOUNT}/preprocessData/cluster-log/${JOB_NAME}.out
              --error=${DIR_TO_MOUNT}/preprocessData/cluster-log/${JOB_NAME}.err"
